...
# --- Headless Flask Streaming ---
app = Flask(__name__)
latest_eye_frame = None
latest_eye_overlay_frame = None
latest_ext_frame = None
lock = threading.Lock()

def process_camera_headless(cam_index=0, external_index=2):
    global circle_x, circle_y, calibrated, latest_eye_frame, latest_ext_frame, latest_eye_overlay_frame
    ...
    if HIGH_FPS_MODE:
        ...
        eye_cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
        eye_cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
    ...
    while True:
        ...
        _, overlay_frame = process_frame(eye_frame)  
        
        with lock:
            latest_eye_frame = eye_frame.copy()
            latest_eye_overlay_frame = overlay_frame.copy()
        ...
        if external_cap is not None:
            ...
            if ret_ext:
                ...
                with lock:
                    latest_ext_frame = ext_frame_resized.copy()

        time.sleep(0.01)

def generate_frames(camera_type):
    global latest_eye_frame, latest_ext_frame, latest_eye_overlay_frame
    while True:
        with lock:
            if camera_type == 'eye':
                frame = latest_eye_frame
            elif camera_type == 'eye_overlay':
                frame = latest_eye_overlay_frame
            else:
                frame = latest_ext_frame
                
            if frame is None:
                # Provide a blank frame if none exists yet
        ...
        time.sleep(0.03) # Limit framerate over HTTP
...
@app.route('/cam1')
def video_feed_ext():
    return Response(generate_frames('ext'), 
                    mimetype='multipart/x-mixed-replace; boundary=frame')

@app.route('/cam2')
def video_feed_eye_overlay():
    return Response(generate_frames('eye_overlay'), 
                    mimetype='multipart/x-mixed-replace; boundary=frame')
    ...
    <html>
      ...
      <body style="font-family: sans-serif;">
        ...
        <div style="display: flex; flex-wrap: wrap; gap: 20px;">
          ...
          <div>
            <h3>External Camera (Scene + Gaze)</h3>
            <img src="/cam1" width="640" height="480">
          </div>
        </div>
        <div style="margin-top: 20px;">
            <button onclick="fetch('/calibrate').then(()=>alert('Calibrated!'))" style="padding: 10px; font-size: 16px;">Trigger Calibration</button>
        </div>
      </body>
    </html>
    '''
