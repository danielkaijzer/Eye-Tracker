"""Capture ChArUco frames from the eye cam and solve for intrinsics.

Mirrors calibrate_scene_intrinsics.py, retuned for the eye camera (lower
resolution, IR, often mounted inverted). Intrinsics are a property of the lens
itself, so the rig/jig is irrelevant here — just wave the board in front of the
eye cam.

IMPORTANT: calibrate in the SAME optical configuration you'll actually use
(IR filter on/off, and the focus you lock for extrinsic capture). If you change
focus, the intrinsics change — recapture for that focus. See the witness-mark
note in the extrinsics workflow.

Workflow:
1. Run scripts/extras/generate_charuco_board.py and display its output, or
   print it. The eye cam must see it sharply at whatever focus you're testing.
2. Run this script. Preview shows the feed with detections overlaid.
3. Vary eye-cam pose: close/far, tilted, rotated, board near the image corners.
   Press SPACE on each pose to capture. Aim for 15+ varied captures.
4. Press C to compute. R to reset. Q to quit.

Output: scripts/eyetracker/eye_intrinsics.npz with K, dist, image_size,
reproj_rms, timestamp — the format the extrinsics script's --eye-intrinsics
expects.
"""
import argparse
import math
...
from scripts.eyetracker.config import EYE_CAM_RESOLUTION

# Same board as calibrate_scene_intrinsics.py so one generated board serves both.
...
OUTPUT_PATH = "scripts/eyetracker/eye_intrinsics.npz"
...
def main():
    ...
    parser.add_argument("--cam-index", type=int, default=0,
                        help="OpenCV camera index for the eye cam")
    parser.add_argument("--flip", action="store_true",
                        help="flip vertically (eye cam mounted inverted)")
    ...
    print(f"Eye cam: {cam.width}x{cam.height}")
