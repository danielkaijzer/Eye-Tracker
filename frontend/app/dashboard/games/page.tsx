...
export default function GamesPage() {
    ...
    const [statusMessage, setStatusMessage] = useState("Press start to begin the grid challenge.");
    ...
    async function handleStart() {
        ...
        try {
            if (document.fullscreenElement !== gameRef.current) {
                ...
                // Calculate grid size after entering fullscreen
                setTimeout(() => {
                    const newGridSize = calculateGridSize(
                        window.innerWidth,
                        window.innerHeight
    ...
    function handleCellClick(index: number) {
        ...
        setTargetIndex((previousTarget) => createTarget(previousTarget, gridSize * gridSize));
    ...
    async function handleExit() {
        setStarted(false);
        setBestScore((currentBest) => Math.max(currentBest, score));
        setStatusMessage(`Game ended. Final score: ${score}.`);
    ...
    return (
        <div ref={gameRef} className="flex min-h-screen flex-col bg-[#090909] font-sans text-white">
            <header className="flex items-center justify-between border-b border-white/10 px-4 py-4 sm:px-6">
                ...
                <div className="text-center">
                    <p className="text-xs uppercase tracking-[0.4em] text-zinc-500">Games</p>
            ...
            <main className="flex flex-1 items-center justify-center p-4 sm:p-6">
                <div className="grid w-full max-w-5xl gap-6 lg:grid-cols-[320px_1fr]">
                    <section className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-2xl shadow-black/30">
                        <p className="text-xs uppercase tracking-[0.35em] text-zinc-500">Control Panel</p>
                        <h2 className="mt-2 text-3xl font-semibold">Stay sharp.</h2>
                        <p className="mt-4 text-sm leading-relaxed text-zinc-400">
                            Click the highlighted cell as fast as you can. Press Start to take over the
                            full screen with only the grid.
                        </p>

                        <div className="mt-6 grid grid-cols-3 gap-3 text-center">
                            <div className="rounded-2xl border border-zinc-800 bg-zinc-950/80 p-3">
                                <p className="text-[11px] uppercase tracking-[0.2em] text-zinc-500">Score</p>
                                <p className="mt-1 text-2xl font-semibold">{score}</p>
                            </div>
                            <div className="rounded-2xl border border-zinc-800 bg-zinc-950/80 p-3">
                                <p className="text-[11px] uppercase tracking-[0.2em] text-zinc-500">Best</p>
